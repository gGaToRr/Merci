...
import { Play, Plus, X, Film, Tv, Star, Eye, Loader2, Search, AlertTriangle,
         ...
         ChevronLeft, ChevronRight, Maximize2 } from 'lucide-react'
...
function getEmbedUrl(hoster: string, rawInput: string): string | null {
  ...
  if (['autoembed','vidsrc','vidsrcme','videasy','twoembed'].includes(hoster)) {
...
const ALL_TAGS = [
  ...
  'Fantasy','Romance','Animé','Seinen','Shonen','Historique','Crime','Documentaire',
  'Famille','Musique','Mystère','Western','Guerre','Sport','Biopic',
]
const HOSTERS_LIST = ['autoembed','vidsrc','vidsrcme','videasy','twoembed','vidoza','doodstream','streamtape','voe','mixdrop','filemoon','sendvid','sibnet','myvidplay','odysee','uptobox','autre']
        ...
        <div ref={containerRef} className="flex-1 bg-black relative" onClick={() => setOpen(false)}>
          ...
          {!loadingPlayer && (
            <button onClick={nextEpisode}
              className="absolute bottom-4 right-4 flex items-center gap-2 px-4 py-2.5 rounded-xl bg-teal-600/90 hover:bg-teal-500 text-white text-xs font-semibold shadow-xl transition-all hover:scale-105 backdrop-blur-sm z-10">
              Épisode suivant <ChevronRight className="w-4 h-4" />
            </button>
          )}
        </div>
      )}
...
function ContentCard({ item, isAdmin, onPlay, onEdit, onDelete }: {
  ...
  return (
    <div className="bg-[#11101e] border border-gray-800/60 rounded-xl sm:rounded-2xl overflow-hidden hover:border-teal-600/40 hover:shadow-xl hover:shadow-teal-950/20 transition-all duration-300 group flex flex-col">
...
export function TheGoodPlace() {
  ...
  const handlePlay = async (item: Content, link: ContentLink) => {
    ...
    // Séries et animés → sélecteur d'épisodes
    if (item.content_type === 'serie' || item.content_type === 'anime') {
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          ...
          {isAdmin && (
            <button onClick={handleAdd}
              className="flex items-center gap-1.5 px-3 sm:px-4 py-2 sm:py-2.5 bg-teal-600/20 border border-teal-600/40 hover:bg-teal-600/30 rounded-xl text-teal-400 text-xs sm:text-sm font-medium transition-all shadow-lg shadow-teal-900/20">
        ...
        {loading ? (
          ...
          <>
            <p className="text-[10px] text-gray-600 uppercase tracking-wider mb-3 sm:mb-4">
              {filtered.length} {section==='film'?'film':section==='serie'?'série':'animé'}{filtered.length>1?'s':''}
