...
def load_config(config_abs):
    ...
    expected = ['DEFAULT', 'mpd', 'download_dirs', 'debug', 'http', 'proxy', 'threadpool', 'deezer', 'youtubedl', 'movies']
    actual = list(config.keys())
    assert actual == expected or actual == expected[:-1], f"Validating config file failed. Check {config_abs}"
