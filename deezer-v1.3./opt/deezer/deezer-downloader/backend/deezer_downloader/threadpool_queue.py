    ...
    def get_queue(self):
        ...
        for t in self.all_tasks:
            tasks.append({
                ...
                "status": state_map.get(t.state, t.state),
                "progress": getattr(t, "progress", 100 if t.state == "mission accomplished" else 0),
                "error": str(t.exception) if t.exception else None,
            })
        return {
            "queue": list(reversed(tasks[-50:])),
...
def report_progress(value, maximum):
    ...
    def get_queue(self):
        ...
        return {
            'queue': tasks,
            'workers': len(getattr(self, '_workers', [])),
        }
